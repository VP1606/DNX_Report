...
\noindent Chapter~\ref{cha:background} provides the theoretical context for the \texttt{DetectorNX} project. It surveys \glspl{cnn} for object detection, introduces \glspl{snn} via the \gls{lif} neuron and surrogate gradients, and reviews event-based vision alongside the \gls{etram} dataset. It then characterises neuromorphic energy economics, concluding with a collection of related work that identifies the critical gap in automotive-specific spiking object detection at high-definition.
