...
This project has systematically deconstructed the performance-efficiency frontier of Spiking Neural Networks (\glspl{snn}) for high-definition automotive object detection. By evaluating the \texttt{DetectorNX ULTRA} architecture against a matched continuous-valued \gls{cnn} baseline on the ETraM dataset \parencite{Verma_2024_CVPR}, several definitive outcomes have been established that validate the neuromorphic paradigm for safety-critical edge deployment.

\subsection{Global Precision Parity}
The primary outcome of the spatial evaluation was the achievement of a \gls{miou} of $0.4613$ for the spiking model, representing $94.9\%$ of the \gls{cnn} baseline ($0.4861$). 
...
\vspace{1em}
In summary, the empirical evidence demonstrates that the \texttt{DetectorNX ULTRA} has successfully bridged the precision gap, establishing the spiking paradigm not merely as a theoretical curiosity, but as a robust, first-class solution for high-definition, power-constrained automotive perception.
