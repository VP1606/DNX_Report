...
\begin{itemize}
    ...
    \item \textbf{Hardware Extrapolability:} While constrained to local GPU execution, the benchmark outputs (e.g., dynamic power tracking) were required to be translatable into theoretical energy consumption metrics (in mJ) based on established hardware constants, facilitating projections for specialized neuromorphic chips like Intel Loihi or IBM TrueNorth.
\end{itemize}

These requirements formed the foundation for the three-generation evolution of the \texttt{SNN\_MAX\_REPLICA\_SE} architecture detailed in Section \ref{sec:snn_architecture}, and drove the rigorous benchmarking suite evaluated in Section ?? %\ref{sec:results_evaluation}%.
\todo[inline]{Uncomment line once references are resolved}
 \todo[inline,backgroundcolor=myblue,textcolor=white]{Check \& Proof-read this section against rubric.}