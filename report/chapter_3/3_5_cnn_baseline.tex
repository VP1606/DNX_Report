...
The \gls{cnn} network encounters the same informational bottlenecks as its \gls{snn} counterpart, detailed in Section \ref{subsec:snn_se_block}. Consequently, it implements the same architectural solution via an \gls{se} Block \cite{hu2019_se_block}. However, the \texttt{CNN\_MAX\_REPLICA\_SE} implements this mechanism in the continuous domain: as opposed to utilizing spike rates, the \gls{cnn} employs a float-based approach.

During the `Squeeze' segment, the \gls{cnn} utilizes global average pooling to calculate the mean spatial intensity of features per channel, rather than calculating a temporal firing rate. Subsequently, during the `Excitation' phase, the block outputs a learned coefficient that scales the input activations. This effectively recalibrates the channel-wise feature responses, as opposed to modulating discrete membrane potentials.

While the integration of an \gls{se} Block in a standard \gls{cnn} improves feature representation, it does not confer the same critical hardware-level energy sparsity benefits as it does within an \gls{snn}. The primary advantage of the \gls{se} mechanism in this specific context is the enforcement of structural sparsity in the spiking domain. Nevertheless, to strictly adhere to the Architectural Parity requirement established in Section \ref{subsec:req_functional}, the continuous equivalent of this mechanism was preserved in the baseline.

% Furthermore, a Squeeze-and-Excitation (SE) block is integrated at the bottleneck (between Blocks 2 and 3). The SE block performs global average pooling to recalibrate channel-wise features, effectively allowing the network to ``pay attention'' to the most informative feature maps within the event stream, thereby improving feature discriminability in complex multi-object scenes \cite{hu2018squeeze}.
