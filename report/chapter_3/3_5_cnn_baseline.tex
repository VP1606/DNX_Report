...
\begin{figure}[htbp]
    ...
    \caption[End-to-end \texttt{CNN\_MAX\_REPLICA\_SE} architecture.]{End-to-end data flow through the \texttt{CNN\_MAX\_REPLICA\_SE} baseline. The four backbone stages mirror those of the \gls{snn} (Figure~\ref{fig:snn_architecture}) 1:1, differing only in activation (\gls{relu} instead of \gls{lif}), residual formulation (Standard ResBlock instead of \texttt{SEWResBlock}), and temporal handling: the $T = 10$ bins are channel-stacked into a 20-channel input and processed in a single spatial pass, removing the need for a Temporal Fusion stage.}
