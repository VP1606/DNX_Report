...
The \gls{cnn} backbone employs deep residual learning (Section~\ref{sec:background_cnn}), utilizing four Standard Residual Blocks that expand to 512 channels in the final stage. To satisfy Architectural Parity, the \texttt{SEWResBlock}s of the \gls{snn} were replaced with Standard Residual Blocks of the structure $\text{Conv} \rightarrow \text{BN} \rightarrow \text{ReLU} \rightarrow \text{Conv} \rightarrow \text{BN} + \text{Shortcut} \rightarrow \text{ReLU}$; these shortcut paths mitigate vanishing gradients and enable stable training at this depth \cite{he2015deep}.
...
If the corresponding SNN achieves 43\% \gls{iou}, the 5\% gap explicitly represents the ``quantization cost'' of switching from 32-bit continuous floats to 1-bit binary spikes \cite{cordone2022objectdetectionspikingneural}. 
