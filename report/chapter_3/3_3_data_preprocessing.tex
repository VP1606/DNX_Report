...
The time window $\Delta T = 100$\,ms was chosen to align with the human visual system's $\sim$100\,ms temporal persistency \cite{deodato2024continuous}; the \gls{snn} integrates the asynchronous event stream over this duration before producing a detection output, exactly like the human eye.
...
Second, specific to the context of vehicle detection, it is short enough that a vehicle travelling at highway speed (e.g., $120$\,km/h $\approx 33$\,m\,s$^{-1}$) displaces by
only $3.3$\,m within the window, ensuring that the bounding box annotation
...
A naive dataloader would stream the raw h5 and npy files in whatever segment fits in memory and re-run the full pipeline (signal conditioning, voxelisation, downsampling, label alignment) on each pass, multiplying the preprocessing cost 150-fold and starving the \gls{gpu} while the \gls{cpu} works through every voxelisation and denoising call.
...
Figure \ref{fig:preprocessing_pipeline} details the order in which the aforementioned preprocessing steps are applied to the raw event data.
