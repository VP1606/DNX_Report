...
To maximize training efficiency and ensure a high signal-to-noise ratio in the training distribution, the \gls{bolt} pipeline adopts a Box-Central sampling strategy rather than more conventional uniform temporal partitioning, where training windows were anchored deterministically around the centroids of
...
In combination with the Box-Central sampling approach, the pipeline applies a density filter prior to voxelization: for each $100\text{ms}$ window the raw event count is checked, and windows containing fewer than \texttt{min\_events} $= 500$ events are discarded. 
...
The first step of the \gls{bolt} pipeline applies a series of signal-conditioning operations, lifted directly from the original \gls{etram} study since they were tuned to the characteristics of the dataset \cite{Verma_2024_CVPR}, and serving three complementary purposes.
