...
The raw event data from the \gls{etram} dataset takes the form $e = (x, y, t, p)$, as defined in Section~\ref{subsec:bg_event_camera},
which was provided as pre-recorded \texttt{.h5} event files at $1280 \times 720$ spatial resolution.
However, raw event streams are inherently sparse, noisy and incompatible with conventional deep learning frameworks that expect dense, structured tensors.
To bridge this gap, the raw \gls{etram} data was passed through a preprocessing pipeline before network ingestion.
...
To make the asynchronous event data compatible with neural network layers, the Voxelization process introduced in Section \ref{subsec:bg_voxelization} is performed to transform the continuous stream into a structured tensor.
In this pipeline, voxelization was implemented using $T=10$ discrete time bins, encompassing a total window duration of $\Delta T = 100 \text{ms}$, ensuring that the data is frequent enough to be able to track moving cars without significant gaps in their motion history.
The result of the voxelization process is a dense 4D spatiotemporal tensor of shape $[10, 2, 720, 1280]$ (representing Time $\times$ Channels $\times$ Height $\times$ Width).
