...
This section specifies how the \texttt{SNN\_MAX\_REPLICA\_SE} architecture established in Section~\ref{sec:snn_architecture} is trained on the precomputed voxel chunks produced by the \texttt{REPLICA} pipeline (Section~\ref{sec:data_preprocessing}).
...
\begin{table}[htbp]
\centering
\caption[Training configuration for \texttt{SNN\_MAX\_REPLICA\_SE}.]{Training configuration for the \texttt{SNN\_MAX\_REPLICA\_SE} (``Ultra'') production run.}
