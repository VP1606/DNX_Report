...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.95\textwidth}
        ...
        \caption{Temporal Sequence 1: Initial hunting behavior and edge consolidation.}
...
Each sample in Figure \ref{fig:temporal_sequences} exhibit different behaviours:
...
yet overlapping boxes and non-monotonic confidence scores persist.
This behaviour is consistent with competing spike trains from multiple event sources (i.e. cars) preventing stable threshold-crossing conensus,
...
correctly detecting the car in the bottom right corner with a confidence of $91\%$ at the first timestep $t=2$.
...
% Ultimately, the emergence of stable, high-confidence detections as early as $t=50\text{ms}$ (Sequence 3) confirms the \gls{snn}'s capacity for asynchronous, low-latency inference. While the hunting seen in complex scenes highlights the difficulty of multi-object resolution, the ability to bypass the full $100\text{ms}$ integration window required by the \gls{cnn} provides the empirical foundation for the $50\%$ latency reduction discussed in the following section.