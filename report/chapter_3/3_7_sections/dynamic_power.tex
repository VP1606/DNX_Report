...
%
The energy formulation that follows for both the \gls{cnn} and \gls{snn} architectures is derived from the analytical framework of \textcite{lemaire2022analytical}.
...
\begin{equation}
  E_{CNN} = \sum_{l=1}^{L} (\mathrm{FLOPs}_l \times E_{MAC})
\end{equation}
...
The number of \glspl{flop} is the total number of \glspl{mac} operations required to produce the output feature map; in a \gls{cnn}, this number is static, regardless of what the input contains.
The \glspl{flop} of a \gls{cnn} can be calculated as follows:

\begin{equation}
     \mathrm{FLOPs}_l = (H_{out} \times W_{out}) \times (K \times K) \times C_{in} \times C_{out}
\end{equation}
...
\begin{equation}
\mathrm{SOPs}_l = \mathrm{FLOPs}_l \times \zeta_l \times T
\label{eq:sop_zeta}
\end{equation}

\noindent where $\zeta_l$ represents the average firing rate of layer $l$, and $T$ is the number of temporal simulation steps ($T=10$).
...
Because \glspl{snn} utilise binary activations, the expensive multiplication part of the \gls{mac} operation is omitted, leaving only an \gls{ac} of the weight to the membrane potential.
The total energy per frame ($E_{SNN}$) is defined as:

\begin{equation}
\label{eq:e_snn}
  E_{SNN} = \sum_{l=1}^{L} (\mathrm{SOPs}_l \times E_{AC})
\end{equation}
