...
Traditional \gls{cnn} architectures are characterized by static power consumption; they perform a dense, fixed number of \gls{mac} operations for every forward pass, regardless of the information density within the input scene. 
...
\noindent where $E_{MAC} = 4.6\text{ pJ}$ per 32-bit floating point operation \cite{horowitz20141}.
...
In order to obtain $\zeta_l$, a specialized \texttt{SpikeRateMonitor} tool was developed.
...
Because \glspl{snn} utilize binary activations, the expensive multiplication part of the \gls{mac} operation is omitted, leaving only a \gls{ac} of the weight to the membrane potential.
...
\noindent where $E_{AC} = 0.9\text{ pJ}$ is the energy cost of a single floating-point addition \cite{horowitz20141}.
