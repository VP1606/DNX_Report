...
The Confidence Calibration evaluates predictive reliability by investigating the calibration gap between spiking and continuous architectures. Modern \glspl{cnn} are prone to systematic overconfidence, frequently assigning high objectness scores to inaccurate detections \cite{guo2017calibration}. 
...
The \gls{cnn} baseline is hypothesized to exhibit significant miscalibration due to its continuous activation functions and exact gradient descent, which tend to overfit the confidence head. 
The \gls{snn} is expected to demonstrate a more linear, calibrated trend. 
In the spiking domain, low-quality features generate fewer spikes, naturally leading to lower membrane potentials in the detection head. 
