...
Qualitatively, Figure \ref{fig:qualitative_inference} confirms that the \gls{snn} maintains precise spatial alignment across diverse traffic densities. In Samples 178 and 030, the spiking model resolves vehicle clusters with the same degree of box stability as the \gls{cnn}, validating the effectiveness of the \texttt{DetectorNX} pipeline in preserving temporal context. In summary, the \texttt{DetectorNX-G3-SNN} has successfully bridged the precision gap, establishing a foundation for the subsequent evaluation of its power efficiency and temporal reliability.
