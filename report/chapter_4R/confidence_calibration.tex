...
The \gls{cnn} baseline (Figure \ref{fig:confidence_calibration}, right) exhibits a pronounced \textit{overconfidence bias}: a significant number of validation samples occupy the positive-confidence region ($x > 0.5$) while maintaining an actual \gls{iou} of zero, and never predicts an $IoU=0.0$ event to have a confidence of zero. 
