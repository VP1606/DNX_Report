...
In contrast, the \texttt{DetectorNX-G3-SNN} architecture consumed a mean of only $2,404.98\ \mu J$ per frame, and dipped as low as $2,287.54\ \mu J$ on highly sparse scenes.
...
This proves the reactive scalability of the architecture. While the \gls{cnn} possesses a static slope of exactly $0$ (consuming $42,429\ \mu J$ regardless of whether the frame contains complex traffic or empty asphalt), the \gls{snn} mimics biological neural efficiency by consuming power only in direct proportion to the incoming visual evidence.
