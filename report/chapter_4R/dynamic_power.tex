...
\subsection*{Spike Activation Per Layer}
...
\hline
\end{tabular}
\caption[Layer-Wise Spike Activation Rates]{Mean spike activation rates ($\zeta$) per layer for the \texttt{DetectorNX-G3-SNN} backbone over the 200-sample experimental set.}
\label{tab:spike_activation}
\end{table}

\subsection*{Mean Energy Savings}
\label{subsubsec:mean_energy_savings}

By substituting the empirical layer-wise firing rates ($\zeta$) recorded in Table \ref{tab:spike_activation} into the dynamic power formulation established in Equation \ref{eq:e_snn} (Section \ref{subsec:dynamic_power}), the network's computational operations can be directly translated into theoretical energy consumption in microjoules ($\mu\text{J}$). This mathematical translation reveals a substantial metabolic advantage for the neuromorphic paradigm, directly validating the core efficiency hypothesis of this research.
...
The continuous \gls{cnn} baseline, processing the input uniformly regardless of scene complexity, consumed a massive, static $42{,}429.21\,\mu\text{J}$ per frame with zero variance.
In contrast, the \texttt{DetectorNX-G3-SNN} architecture consumed a mean of only $2{,}404.98\,\mu\text{J}$ per frame, and dipped as low as $2{,}287.54\,\mu\text{J}$ on highly sparse scenes.

\begin{table}[htbp]
\centering
\begin{tabular}{|l|c|c|}
\hline
\textbf{Metric} & \textbf{\texttt{DetectorNX-G3-SNN}} & \textbf{\texttt{DetectorNX-G3-CNN}} \\
\hline
...
\hline
\end{tabular}
\caption[Energy Statistics per Frame]{Dynamic energy statistics ($\mu\text{J}$ per frame) comparing the \gls{snn} and \gls{cnn} architectures over the 200-sample validation set.}
...
\subsection*{Reactive Scalability: The Line of Best Fit}

To empirically validate the hypothesis from Section \ref{subsec:dynamic_power} that the \gls{snn}'s metabolic cost is a dynamic function of activation sparsity, a linear regression analysis was conducted mapping the raw event count ($x$) of each frame against the theoretical energy consumption ($y$ in $\mu\text{J}$).
...
\begin{itemize}
    \item \textbf{The Metabolic Resting State ($y$-intercept):} The constant $2294.80\,\mu\text{J}$ represents the architectural overhead---the baseline energy required to simply maintain the membrane potentials across the 11.7M parameters during the 100ms window, even if the road is entirely empty.
    \item \textbf{The Cost of Information (Slope):} The gradient $0.001757\,\mu\text{J}$ represents the exact, marginal energy cost of processing a single new event (e.g., a pixel of a car moving).
\end{itemize}

This proves the reactive scalability of the architecture. While the \gls{cnn} possesses a static slope of exactly $0$ (consuming $42{,}429\,\mu\text{J}$ regardless of whether the frame contains complex traffic or empty asphalt), the \gls{snn} mimics biological neural efficiency by consuming power only in direct proportion to the incoming visual evidence.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{SNN Dynamic Power ($\mu\text{J}$)}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{CNN Static Power ($\mu\text{J}$)}
    ...
    \caption[Dynamic Power Consumption vs. Scene Activity]{Comparison of dynamic power consumption ($\mu\text{J}$) between the SNN and CNN architectures relative to the raw event count of the input scene. The isolated SNN graph (a) demonstrates strong linear scaling ($R^2 = 0.7514$), validating its event-driven efficiency, whereas the CNN (b) consumes a massive static overhead.}
...
\begin{itemize}
    \item \textbf{\gls{snn} Power:} $(2672.19\times 10^6) \times 0.9\,\text{pJ} = 2404.97\,\mu\text{J}$
    \item \textbf{\gls{cnn} Power:} $(9223.74 \times 10^6) \times 4.6\,\text{pJ} = 42429.20\,\mu\text{J}$
\end{itemize}
