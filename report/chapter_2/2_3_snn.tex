...
\todo[inline]{\texttt{3\_4\_snn\_architecture.tex} §SE-Block --- remove the duplicate \texttt{se\_block\_diagram.pdf} figure (now in Figure~\ref{fig:se_block_bg}) and strip the Squeeze/Excitation mechanism explanation (see Section~\ref{subsec:bg_se}). Retain only the Stage 3 / $22\times40\times256$ application context and the ``Objectness'' framing, with a back-reference to Section~\ref{subsec:bg_se}.}

\todo[inline]{\texttt{3\_4\_snn\_architecture.tex} §FRR --- strip the network silencing/over-saturation motivation and the squared-error equation (both now in Section~\ref{subsec:bg_frr}). Retain only the project-specific $\lambda=5.0$, $\text{Target}=20\%$, and cite, with a back-reference to Section~\ref{subsec:bg_frr}.}
