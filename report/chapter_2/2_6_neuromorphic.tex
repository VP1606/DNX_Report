...
Unlike conventional \glspl{gpu}, which execute computation synchronously across all neurons at every clock cycle, both architectures operate asynchronously and in an event-driven manner.
A neuron consumes energy only when it receives a spike and crosses its firing threshold, remaining entirely silent and drawing no dynamic power in the absence of input activity.
...
An \gls{snn}, by contrast, communicates exclusively via binary spikes; since a spike is either $0$ or $1$, the multiplication is eliminated entirely, reducing each synaptic event to an \gls{ac} operation at a cost of $E_{\text{AC}} = 0.9$\,pJ --- a $5\times$ reduction in per-operation energy at the silicon level (Figure~\ref{fig:mac_vs_ac}) \cite{horowitz20141}.
