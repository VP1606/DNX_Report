...
\subsubsection{What is an Artificial Neural Network?}
...
\subsubsection{Training an Artificial Neural Network}
...
\subsubsection{Multiply-Accumulate Operations}
...
\todo[inline]{§3.4 \texttt{Backward Pass} opening paragraph (lines 1--3) re-explains why backpropagation requires differentiability. Once §2.1 is in place, reduce to a single sentence: ``since the Heaviside function is non-differentiable (§\ref{sec:background_ann}), conventional backpropagation fails immediately'', then proceed to the surrogate solution.}
